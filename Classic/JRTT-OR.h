...
Trie* buildTrie(const vector<int>& array) {
	Trie* trie = new Trie();
...
long long searchTrie(Trie*& trie, int a, int m, int index) {
	if (0 == trie) {
		return 0;
	}
	Trie* cur = trie;
	for (int i = index; i >= 0; --i) {
		...
		if (1 == adigit && 1 == mdigit) {
			...
			long long val0 = (0 == cur->next[0] ? 0 : cur->next[0]->count);
			long long val1 = searchTrie(cur->next[1], a, m, i-1);
			return val0 + val1;
		}
		else if (0 == adigit && 0 == mdigit) {
			long long val0 = searchTrie(cur->next[0], a, m, i-1);
			long long val1 = (0 == cur->next[1] ? 0 : cur->next[1]->count);
			return val0 + val1;
		}
	}
...
long long function(const vector<int>& array, const int& m)
{
	Trie* trieTree = buildTrie(array);
	long long result = 0;
	for(int i = 0; i < (int)array.size(); ++i) {
		result += searchTrie(trieTree, array[i], m, 16);
	}
	return result / 2;
}
